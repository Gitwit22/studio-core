...
function validateEnvVars() {
    ...
    if (!hasLiveKitUrl)
        missing.push("LIVEKIT_URL or LIVEKIT_HTTP_URL");
    if (missing.length > 0) {
...
async function incrementRecordingUsage(uid, minutes) {
...
async function getSignedDownloadUrl(key, expiresIn = 300) {
    const cfg = getR2Config();
    const client = getS3Client();
    const command = new client_s3_1.GetObjectCommand({ Bucket: cfg.bucket, Key: key });
...
router.post("/start", requireAuth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        // Validate request
        const { roomName, layout: rawLayout, mode: rawMode, presetId, usageType: rawUsageType } = req.body;
        if (!roomName) {
        ...
        let activeStreamPresetId = null;
        let hasActiveStream = false;
        try {
            const streamSnap = await firebaseAdmin_1.firestore.collection("activeStreams").doc(streamDocId).get();
            ...
            console.warn("[recordings/start] failed to read active stream preset", e?.message);
        }
        const requestedUsageType = rawUsageType === "live" || rawUsageType === "recording_only" || rawUsageType === "live+recording"
            ? rawUsageType
            : null;
        ...
        const clamp = (0, mediaPresets_1.clampRecordingPreset)(planId, presetId, activeStreamPresetId, allowHigherRecordingThanStream);
        ...
        if (hasActiveStream) {
            try {
                ...
                console.warn("[recordings/start] failed to tag active stream usageType", e?.message);
            }
        }
        // =========================================================================
        ...
        try {
            ...
            if (!s3UploadConfig.endpoint)
                configErrors.push("endpoint is empty");
            if (!s3UploadConfig.accessKey)
                configErrors.push("accessKey is empty");
            if (!s3UploadConfig.secret)
                configErrors.push("secret is empty");
            ...
            console.log("[recordings/start] File output config:", {
                filepath: objectKey,
                fileType: "MP4",
                outputCase: fileOutput?.output?.case,
...
router.get("/emergency-latest", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (snap.empty) {
            ...
            const fallbackSnap = await firebaseAdmin_1.firestore
                .collection("recordings")
                .where("userId", "==", uid)
                .orderBy("createdAt", "desc")
                .limit(5)
...
// =============================================================================
router.get("/:id/storage-check", requireAuth_1.requireAuth, async (req, res) => {
    try {
        const uid = getAuthUserId(req);
...
router.post("/:id/report-download-issue", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        await firebaseAdmin_1.firestore.collection("recordings").doc(recordingId).set({
