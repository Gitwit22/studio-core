...
app.post("/api/auth/signup", async (req, res) => {
  try {
    const { email, password, displayName, planId, timeZone, defaultResolution, defaultDestinations, defaultPrivacy } = req.body as {
      email?: string;
      password?: string;
      displayName?: string;
      planId?: string;
    ...
    const userRef = await db.collection("users").add({
      ...
      defaultDestinations: defaultDestinations || { youtube: false, facebook: false },
...
app.get("/api/usage/summary", async (req, res) => {
  try {
    ...
    // Define plan limits
    const planLimits: Record<string, { maxHours: number; maxGuests: number; multistreamEnabled: boolean }> = {
      free: { maxHours: 5, maxGuests: 2, multistreamEnabled: false },
      starter: { maxHours: 20, maxGuests: 4, multistreamEnabled: true },
      pro: { maxHours: 100, maxGuests: 10, multistreamEnabled: true },
      enterprise: { maxHours: 999, maxGuests: 50, multistreamEnabled: true },
    };

    const planId = userData.plan || "free";
    const limits = planLimits[planId] || planLimits.free;

    // Calculate usage (placeholder - you'd track this in your app)
    const usedHours = userData.usedHours || 0;
    const ytdHours = userData.ytdHours || 0;
    ...
    return res.json({
      ...
      multistreamEnabled: limits.multistreamEnabled,
    });
  } catch (err) {
